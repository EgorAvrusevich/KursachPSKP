...
import { Calendar, Clock, Video, User, Briefcase, ChevronRight, Trash2 } from 'lucide-react';
...
const ProfilePage = () => {
  ...
  const [activeTab, setActiveTab] = useState('settings'); // 'settings' или 'interviews'
    ...
    const fetchProfileData = async () => {
      try {
        const [profileRes, interviewsRes] = await Promise.all([
          ...
          api.get('/interviews/my') // Эндпоинт для получения встреч текущего пользователя
        ]);
  ...
  const handleDelete = async (id, isFuture) => {
    ...
    if (isFuture) {
      // Если пользователь — рекрутер, пишем об отмене для кандидата
      // Если кандидат — об отмене для рекрутера
      confirmMsg = data.role === 'Recruiter'
        ? "Это интервью еще не состоялось. При удалении кандидату придет уведомление об отмене. Продолжить?"
        : "Вы уверены, что хотите отменить интервью? Рекрутер получит уведомление об отмене. Продолжить?";
    } else {
      confirmMsg = "Удалить запись об интервью из истории?";
    }
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || "Не удалось удалить интервью";
      alert(errorMsg);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden">
        {activeTab === 'settings' ? (
          /* ВКЛАДКА НАСТРОЕК */
          <form onSubmit={handleSave} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-300">
          ...
          <div className="p-8 animate-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-4">
              {interviews.length > 0 ? (
                interviews.map((item) => {
                  // 1. Создаем объект даты из строки БД
                  ...
                  // 3. Форматируем время без использования параметра timeZone: 'UTC', 
                  // так как мы уже скорректировали саму дату
